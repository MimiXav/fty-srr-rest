/*  =========================================================================
    fty_srr_rest_selftest.c - run selftests
...
// Tests for stable public classes:
    { "stub_srr_rest", stub_srr_rest_test, true, true, NULL },
...
test_runall (bool verbose)
{
    ...
    for (item = all_tests; item->testname; item++) {
        ...
        if (!item->subtest)
            item->test (verbose);
#ifdef FTY_SRR_REST_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_srr_rest_private_selftest (verbose, item->subtest);
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("fty_srr_rest_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running fty-srr-rest test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef FTY_SRR_REST_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_srr_rest_private_selftest (verbose, test->subtest);
#endif // FTY_SRR_REST_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
