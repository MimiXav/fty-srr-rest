...
if ! ((command -v dpkg-query >/dev/null 2>&1 && dpkg-query --list libfty_common_db-dev >/dev/null 2>&1) || \
       (command -v brew >/dev/null 2>&1 && brew ls --versions fty-common-db >/dev/null 2>&1)); then
    BASE_PWD=${PWD}
    cd tmp-deps
...
if ! ((command -v dpkg-query >/dev/null 2>&1 && dpkg-query --list libfty_common_rest-dev >/dev/null 2>&1) || \
    ...
    BASE_PWD=${PWD}
    cd tmp-deps
    $CI_TIME git clone --quiet --depth 1 -b master https://github.com/42ity/fty-common-rest.git fty-common-rest
    cd fty-common-rest
    CCACHE_BASEDIR=${PWD}
    export CCACHE_BASEDIR
    git --no-pager log --oneline -n1
...
if ! ((command -v dpkg-query >/dev/null 2>&1 && dpkg-query --list libfty_common_dto-dev >/dev/null 2>&1) || \
       (command -v brew >/dev/null 2>&1 && brew ls --versions fty-common-dto >/dev/null 2>&1)); then
    BASE_PWD=${PWD}
    cd tmp-deps
...
if [ -z "$DO_CLANG_FORMAT_CHECK" -a -f configure.ac ]; then
    ...
    $CI_TIME tar -xzf fty-srr-rest-1.0.0.tar.gz
    cd fty-srr-rest-1.0.0
fi
